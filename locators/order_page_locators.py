...
class ButtonsOrder:
    ...
    ORDER_CONFIRMATION = [By.XPATH, './/div[@class = "Order_ModalHeader__3FDaJ"]']  # Окно подтверждения заказа
    BUTTON_YES = [By.XPATH, './/button[text() = "Да"]']  # Кнопка "Да" на странице с формой заказа
    SUCCESS_ORDER = [By.XPATH, './/button[text() = "Посмотреть статус"]']  # Кнопка "Посмотреть статус"
